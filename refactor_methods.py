...
def refactor_methods(line_data):
    """ """
    for i in range(len(line_data)):
        if ':' in line_data[i][2] and 'def' in line_data[i][2]:
            if '__init__' in line_data[i][2]:
                line_data[i][2] = line_data[i][2].replace('__init__', 'constructor')
            if re.search(r'self,(\s*)', line_data[i][2]) is not None:
                self_comma = re.search(r'self,(\s*)', line_data[i][2]).group()
                line_data[i][2] = line_data[i][2].replace(self_comma, "")
                def_ = re.search(r'def\s*', line_data[i][2]).group()
                line_data[i][2] = line_data[i][2].replace(def_, "")
            elif 'self' in line_data[i][2]:
                line_data[i][2] = line_data[i][2].replace('self', '')
                def_ = re.search(r'def\s*', line_data[i][2]).group()
                line_data[i][2] = line_data[i][2].replace(def_, "")
            else:
                line_data[i][2] = line_data[i][2].replace('def', 'export function')
...
if __name__ == "__main__":
    ...
    print_list(refactor_methods(line_data))
