...
def refactor_conditionals(line_data):
    for i in range(len(line_data)):
        if re.match(r'(^\s*)(if|elif|for|while)(\s*)(.*):', line_data[i][2]) is not None:
            match = re.search(r'(^\s*)(if|elif|for|while)(\s*)(.*):', line_data[i][2])
            line_data[i][2] = match.group(1) + match.group(2) + match.group(3) + "(" + match.group(4) + ")" + " {\n"
            if 'elif' in line_data[i][2]:
                line_data[i][2] = line_data[i][2].replace('elif', 'else if')
        elif re.match(r'^\s*else:', line_data[i][2]) is not None:
            match = re.search(r'(^\s*)else:', line_data[i][2])
            line_data[i][2] = match.group(1) + 'else' + ' {\n'
    return line_data
