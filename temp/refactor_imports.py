...
def refactor_imports(line_data):
    ...
    for i in range(len(line_data)):
        line = line_data[i][1]
        if re.match(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line) is not None:
            wspace1 = re.search(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line).group(1)
            filepath = re.search(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line).group(2)
            wspace2 = re.search(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line).group(3)
            wspace3 = re.search(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line).group(4)
            atrib = re.search(r'from(\s*)([a-zA-Z_.]+)(\s*)import(\s*)([a-zA-Z_*]+)', line).group(5)
            line = 'import' + wspace1 + '{ ' + atrib + ' }' + wspace2 + 'from' + wspace3 + '"' + filepath + '"' + ";"
            line = line.replace('.', '/')
            line_data[i] = (line_data[i][0], line)
        elif re.match(r'import(\s*)([a-zA-Z_.]+)', line) is not None:
            wspace = re.match(r'import(\s*)([a-zA-Z_.]+)', line_data[i][1]).group(1)
            filepath = re.match(r'import(\s*)([a-zA-Z_.]+)', line_data[i][1]).group(2)
            line = 'import' + wspace + '"' + filepath + '"' + ';'
            line = line.replace('.', '/')
            line_data[i] = (line_data[i][0], line)
    return line_data
