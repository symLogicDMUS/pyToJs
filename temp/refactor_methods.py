...
def refactor_methods(line_data):
    ...
    for i in range(len(line_data)):
        if ':' in line_data[i][1] and 'def' in line_data[i][1]:
            line = line_data[i][1]
            if '__init__' in line:
                line = line.replace('__init__', 'constructor')
            if re.search(r'self,(\s*)', line) is not None:
                self_comma = re.search(r'self,(\s*)', line).group()
                line = line.replace(self_comma, "")
                def_ = re.search(r'def\s*', line).group()
                line = line.replace(def_, "")
            elif 'self' in line:
                line = line.replace('self', '')
                def_ = re.search(r'def\s*', line).group()
                line = line.replace(def_, "")
            else:
                line = line.replace('def', 'export function')
            line = line.replace(':', ' {')
