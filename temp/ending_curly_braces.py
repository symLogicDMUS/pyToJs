...
def ending_curly_braces(blocks, multiples):
    """ """
    for i in range(len(blocks)):
        if not blocks[i][0][0] == 0 and ':' not in blocks[i][0][1]:
            sspaces = re.search(r'^\s*', blocks[i][0][1]).group()
            num_spaces = blocks[i][0][0]
            end_i = multiples * -1
            blocks[i].append((num_spaces - multiples, sspaces[:end_i] + "}"))
    return blocks
