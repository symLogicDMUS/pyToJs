...
def get_indent_shifts(line_data):
    """ """
    line_data = list(filter(lambda line: not line[2].isspace(), line_data))
    indent_shifts = []
    num_spaces = line_data[0][1]
    for i in range(len(line_data)):
        if line_data[i][1] > num_spaces:
            indent_shifts.append(('+', line_data[i][0]))
            num_spaces = line_data[i][1]
        elif line_data[i][1] < num_spaces:
            indent_shifts.append(('-', line_data[i][0]))
            num_spaces = line_data[i][1]
    return indent_shifts
...
if __name__ == "__main__":
    ...
    print_list(get_indent_shifts(line_data))
