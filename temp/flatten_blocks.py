...
def flatten_blocks(blocks):
    """ """
    blocks = list(map(lambda block: list(flatten(block)), blocks))
    blocks = list(map(lambda block: list(filter(lambda e: type(e) != int, block)), blocks))
    blocks = list(map(lambda block: "".join(block), blocks))
    return blocks
