...
def refactor_for_in(line_data):
    """" """
    for i in range(len(line_data)):
        if re.search(r'for\s+\((.*) in', line_data[i][2]) is not None:
            match = re.search(r'for\s+\((.*) in', line_data[i][2]).group()
            item1 = re.search(r'for\s+\((.*) in', line_data[i][2]).group(1)
            line_data[i][2] = line_data[i][2].replace(match, 'for (var {} of'.format(item1))
    return line_data
