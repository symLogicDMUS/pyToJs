...
def refactor_dict_methods(line_data):
    """refactor traversing over a python dict keys into traversing over a javascript Object keys"""
    for i in range(len(line_data)):
        if re.search(r'([a-zA-Z_][a-zA-Z0-9_().]+).(keys|values|items)\(\)', line_data[i][2]) is not None:
            match = re.search(r'([a-zA-Z_][a-zA-Z0-9_().]+).(keys|values|items)\(\)', line_data[i][2]).group()
            dict_ = re.search(r'([a-zA-Z_][a-zA-Z0-9_().]+).(keys|values|items)\(\)', line_data[i][2]).group(1)
            method = re.search(r'([a-zA-Z_][a-zA-Z0-9_().]+).(keys|values|items)\(\)', line_data[i][2]).group(2)
            if method == 'items':
                method = 'entries'
            line_data[i][2] = line_data[i][2].replace(match, 'Object.{}({})'.format(method, dict_))
    return line_data
