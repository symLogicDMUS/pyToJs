...
def refactor_conditional_in_operator(line_data):
    """ """
    for i in range(len(line_data)):
        if re.search(r'if \((.*) in (.*)\)', line_data[i][2]) is not None:
            line = re.search(r'if \((.*) in (.*)\)', line_data[i][2]).group()
            item1 = re.search(r'if \((.*) in (.*)\)', line_data[i][2]).group(1)
            item2 = re.search(r'if \((.*) in (.*)\)', line_data[i][2]).group(2)
            line_data[i][2] = line_data[i][2].replace(line, 'if ({}.includes({}))'.format(item2, item1))
    return line_data
...
if __name__ == "__main__":
    line_data = \
        [
            ...
            [38, 8, '        pawn_histories = {}\n'],
            [39, 8, '        for (hist in this.pawn_histories.values()) {\n'],
            ...
            [55, 8, '        pawn_histories = {}\n'],
            [56, 8, '        for (hist in this.pawn_histories.values()) {\n'],
            ...
            [270, 8, '        for (k, v in this.rooks_moved.items()) {\n'],
            [271, 12, '            str_ += str(k)\n'],
            [272, 12, "            str_ += ':'\n"],
            [273, 12, '            str_ += str(v)\n'],
            [274, 12, "            str_ += '\\n'\n"],
            ...
            [278, 8, '        for (k, v in this.kings_moved.items()) {\n'],
            [279, 12, '            str_ += str(k)\n'],
            [280, 12, "            str_ += ':'\n"],
            [281, 12, '            str_ += str(v)\n'],
            [282, 12, "            str_ += '\\n'\n"],
            ...
            [286, 8, '        for (k, v in this.pawn_histories.items()) {\n'],
            [287, 12, '            str_ += str(k)\n'],
            [288, 12, "            str_ += ':'\n"],
            [289, 12, '            str_ += str(v)\n'],
            [290, 12, "            str_ += '\\n'\n"],
    ...
    print_list(refactor_conditional_in_operator(line_data))
