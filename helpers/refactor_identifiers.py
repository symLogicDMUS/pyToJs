import re
import os
from helpers.get_file_str import get_file_str
from helpers.get_line_data import get_line_data
from helpers.print_list import print_list


def refactor_identifiers(line_data, file_name, keyword='let'):
    """ """
    file_str = get_file_str(file_name, line_data)
    matches = set(re.findall(r'([a-zA-Z_][a-zA-Z0-9_]*\s+=[^=])', file_str))
    for match in matches:
        file_str = file_str.replace(match, keyword + " " + match, 1)
    file_str = file_str.replace('self', 'this')
    file_str = file_str.replace('this.let ', 'this.')
...
if __name__ == "__main__":
    ...
    [
        ...
        [54, 8, '        """swap the key of each pawn_history entry with the coordinate of its current location"""\n'],
        [55, 8, '        pawn_histories = {}\n'],
        ...
        [164, 12, '            if npck > 0:\n'],
        [165, 16, "                self.condition, self.game_status, self.winner = 'checkmate', OVER, enemy_color\n"],
        [166, 12, '            }\n'],
        [167, 12, '            else:\n'],
        ...
        [206, 16, "                'pawn_histories': self.pawn_histories, 'last_pawn_move': self.last_pawn_move,\n"],
        ...
        [249, 8, '        """return true if the queen side rook of the given color has moved, otherwise false"""\n'],
        [250, 8, "        if color == 'W':\n"],
    ...
    line_data, line_data_dict = refactor_identifiers(line_data, 'JsonRecords')
